...
class AIController:
    ...
    def __init__(self, ai_character, difficulty='medium'):
        ...
        self.current_state = fsm.DEFAULT_STATE  # Keep for display/debugging
        
    def update_state(self, opponent_character):
        """
        Update AI state for display/debugging purposes (FSM kept for compatibility).
        Main decision-making now uses fuzzy logic.
        
        Args:
            opponent_character: Opponent (player) character object
        """
        # Keep FSM state updated for display/debugging
        # This is optional - fuzzy logic doesn't strictly need it
        try:
    ...
    def select_action(self, opponent_character):
        ...
        for action, prob in action_probs.items():
            if action == 'special':
                ...
                if self.ai_character.can_use_special():
                    ...
                    # If special is on cooldown, redistribute its probability
                    cooldown_penalty = 0.1  # Reduce probability significantly
                    available_actions[action] = prob * cooldown_penalty
            elif action == 'rest':
        ...
        # If no actions available, default to rest or block
        if not available_actions:
            if self.ai_character.stamina < self.ai_character.max_stamina * 0.3:
                return 'rest'
            if self.ai_character.stamina >= config.BLOCK_STAMINA_COST:
                return 'block'
            return 'rest'
        ...
        # Update FSM state for display/debugging (optional)
    ...
    def execute_action(self, action, opponent_character):
        ...
        # Add state information to result (for display/debugging)
    ...
    def get_state_info(self):
        ...
        return {
            ...
            'state_description': self.current_state.description,
