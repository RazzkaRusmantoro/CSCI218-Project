"""
Character definitions for the Turn-Based Fighter Game.
Contains 5 playable characters with their unique special moves and stats.
"""


class Character:
    """Base class for all characters in the game."""
    
    def __init__(self, name, max_hp, max_stamina, base_damage, special_move_name):
        """
        Initialize a character.
        
        Args:
            name: Character name
            max_hp: Maximum health points
            max_stamina: Maximum stamina points
            base_damage: Base damage for basic attacks
            special_move_name: Name of the character's special move
        """
        self.name = name
        self.max_hp = max_hp
        self.hp = max_hp
        self.max_stamina = max_stamina
        self.stamina = max_stamina
        self.base_damage = base_damage
        self.special_move_name = special_move_name
        self.is_blocking = False
        self.is_evading = False
        self.status_effects = {}  # Dictionary of active status effects
        self.special_move_cooldown = 0  # Turns remaining until special can be used
        self.turns_since_special = 0  # Turns since last special move use
        self.consecutive_punches = 0  # Track consecutive punches (for fatigue)
        self.consecutive_kicks = 0  # Track consecutive kicks (for fatigue)
        self.last_move = None  # Track last move (for momentum)
        self.move_variety_bonus = 0.0  # Bonus for move variety (momentum system)
        self.can_counter_attack = False  # Can counter-attack after successful block/evade
    
    def reset_status(self):
        """Reset status effects for a new turn."""
        self.is_blocking = False
        self.is_evading = False
        self.can_counter_attack = False  # Reset counter-attack opportunity
    
    def can_use_special(self):
        """Check if special move can be used (cooldown and stamina)."""
        return self.special_move_cooldown == 0
    
    def use_special_move_with_cooldown(self, target):
        """
        Execute special move and apply cooldown.
        
        Args:
            target: The opponent character to attack
            
        Returns:
            dict: Result of the special move
        """
        if not self.can_use_special():
            return {
                'success': False,
                'message': f"{self.name}'s {self.special_move_name} is on cooldown! ({self.special_move_cooldown} turns remaining)"
            }
        
        result = self.use_special_move(target)
        
        # Apply cooldown if move was successful
        if result.get('success', False):
            from src.utils import config
            self.special_move_cooldown = config.SPECIAL_MOVE_COOLDOWN_TURNS
            self.turns_since_special = 0
        
        return result
    
    def tick_cooldowns(self):
        """Decrease cooldowns each turn."""
        if self.special_move_cooldown > 0:
            self.special_move_cooldown -= 1
        self.turns_since_special += 1
    
    def apply_status_effect(self, effect_type, damage=0, turns=0, **kwargs):
        """
        Apply a status effect to the character.
        
        Args:
            effect_type: Type of status effect (e.g., 'poison', 'burn', 'stun')
            damage: Damage per turn (if applicable)
            turns: Duration in turns
            **kwargs: Additional effect properties
        """
        self.status_effects[effect_type] = {
            'damage': damage,
            'turns': turns,
            **kwargs
        }
    
    def has_status_effect(self, effect_type):
        """Check if character has a specific status effect."""
        return effect_type in self.status_effects
    
    def process_status_effects(self):
        """
        Process all status effects at the start of turn.
        
        Returns:
            dict: Summary of status effects processed (damage dealt, effects expired, etc.)
        """
        total_damage = 0
        expired_effects = []
        
        for effect_type, effect_data in list(self.status_effects.items()):
            # Process damage if applicable
            if effect_data.get('damage', 0) > 0:
                damage = effect_data['damage']
                self.hp = max(0, self.hp - damage)
                total_damage += damage
            
            # Decrease turn counter
            effect_data['turns'] -= 1
            
            # Remove effect if expired
            if effect_data['turns'] <= 0:
                expired_effects.append(effect_type)
                del self.status_effects[effect_type]
        
        return {
            'damage': total_damage,
            'expired_effects': expired_effects,
            'active_effects': list(self.status_effects.keys())
        }
    
    def _apply_damage_with_block_check(self, target, damage, move_name):
        """
        Helper function to apply damage and check for blocking.
        
        Args:
            target: Target character
            damage: Base damage amount
            move_name: Name of the move (for messages)
            
        Returns:
            tuple: (actual_damage, was_blocked_completely, message)
        """
        was_blocking = target.is_blocking
        actual_damage = target.take_damage(damage)
        
        if was_blocking and actual_damage == 0:
            return (0, True, f"{self.name} uses {move_name}, but {target.name} completely blocks it!")
        elif was_blocking and actual_damage < damage:
            return (actual_damage, False, f"{self.name} uses {move_name}! {target.name} blocks, takes {actual_damage} damage (reduced from {damage})!")
        else:
            return (actual_damage, False, f"{self.name} uses {move_name}! Deals {actual_damage} damage!")
    
    def use_special_move(self, target):
        """
        Execute the character's special move.
        Must be implemented by each character subclass.
        
        Args:
            target: The opponent character to attack
            
        Returns:
            dict: Result of the special move with damage, effects, etc.
        """
        raise NotImplementedError("Each character must implement use_special_move")
    
    def restore_stamina(self, amount):
        """Restore stamina points."""
        self.stamina = min(self.max_stamina, self.stamina + amount)
    
    def restore_hp(self, amount):
        """Restore health points."""
        self.hp = min(self.max_hp, self.hp + amount)
    
    def take_damage(self, damage):
        """
        Apply damage to the character.
        Blocking has a chance to completely negate damage, otherwise reduces it significantly.
        """
        if self.is_blocking:
            import random
            from src.utils import config
            # Chance to completely block
            if random.random() < config.BLOCK_COMPLETE_BLOCK_CHANCE:
                return 0  # Complete block - no damage
            # Otherwise reduce damage significantly
            damage = int(damage * (1.0 - config.BLOCK_DAMAGE_REDUCTION))
        self.hp = max(0, self.hp - damage)
        return damage
    
    def is_alive(self):
        """Check if character is still alive."""
        return self.hp > 0
    
    def get_stats(self):
        """Get current character stats as a dictionary."""
        return {
            'name': self.name,
            'hp': self.hp,
            'max_hp': self.max_hp,
            'stamina': self.stamina,
            'max_stamina': self.max_stamina,
            'special_move': self.special_move_name
        }


class Warrior(Character):
    """Balanced fighter with moderate damage."""
    
    def __init__(self):
        super().__init__(
            name="Warrior",
            max_hp=100,
            max_stamina=80,
            base_damage=15,
            special_move_name="Power Strike"
        )
    
    def use_special_move(self, target):
        """Power Strike: Deals high damage, costs moderate stamina."""
        stamina_cost = 30
        if self.stamina < stamina_cost:
            return {'success': False, 'message': f"{self.name} doesn't have enough stamina!"}
        
        self.stamina -= stamina_cost
        damage = int(self.base_damage * 2.5)  # 37-38 damage
        
        # Check if target evades
        if target.is_evading:
            return {'success': False, 'message': f"{target.name} evaded the attack!", 'stamina_cost': stamina_cost}
        
        # Apply damage with block check
        actual_damage, blocked_completely, message = self._apply_damage_with_block_check(target, damage, "Power Strike")
        
        return {
            'success': True,
            'damage': actual_damage,
            'message': message,
            'stamina_cost': stamina_cost
        }


class Tank(Character):
    """High HP, slow, blocks and counters effectively."""
    
    def __init__(self):
        super().__init__(
            name="Tank",
            max_hp=150,
            max_stamina=60,
            base_damage=18,
            special_move_name="Shield Slam"
        )
    
    def use_special_move(self, target):
        """Shield Slam: High damage and blocks next attack."""
        stamina_cost = 35
        if self.stamina < stamina_cost:
            return {'success': False, 'message': f"{self.name} doesn't have enough stamina!"}
        
        self.stamina -= stamina_cost
        self.is_blocking = True  # Shield Slam also blocks next attack
        
        damage = int(self.base_damage * 2.0)  # 36 damage
        
        # Check if target evades
        if target.is_evading:
            return {'success': False, 'message': f"{target.name} evaded the attack!", 'stamina_cost': stamina_cost}
        
        # Apply damage with block check
        actual_damage, blocked_completely, damage_msg = self._apply_damage_with_block_check(target, damage, "Shield Slam")
        
        return {
            'success': True,
            'damage': actual_damage,
            'message': f"{damage_msg} {self.name} also raises guard!",
            'stamina_cost': stamina_cost
        }


class Assassin(Character):
    """High crit chance, ignores block."""
    
    def __init__(self):
        super().__init__(
            name="Assassin",
            max_hp=75,
            max_stamina=90,
            base_damage=16,
            special_move_name="Shadow Strike"
        )
    
    def use_special_move(self, target):
        """Shadow Strike: High crit chance, ignores block."""
        stamina_cost = 30
        if self.stamina < stamina_cost:
            return {'success': False, 'message': f"{self.name} doesn't have enough stamina!"}
        
        self.stamina -= stamina_cost
        
        # Check if target evades
        if target.is_evading:
            return {'success': False, 'message': f"{target.name} evaded the attack!", 'stamina_cost': stamina_cost}
        
        import random
        # 60% crit chance
        is_crit = random.random() < 0.6
        base_damage = int(self.base_damage * 2.0)  # 32 base
        
        if is_crit:
            damage = int(base_damage * 1.8)  # ~57 crit damage
            crit_msg = " CRITICAL HIT!"
        else:
            damage = base_damage
            crit_msg = ""
        
        # Shadow Strike ignores block (assassin specialty)
        was_blocking = target.is_blocking
        target.is_blocking = False
        actual_damage = target.take_damage(damage)
        target.is_blocking = was_blocking
        
        return {
            'success': True,
            'damage': actual_damage,
            'message': f"{self.name} uses Shadow Strike{crit_msg}! Deals {actual_damage} damage (ignores block)!",
            'stamina_cost': stamina_cost
        }


class Mage(Character):
    """Ranged attacks, ignores dodge chance."""
    
    def __init__(self):
        super().__init__(
            name="Mage",
            max_hp=70,
            max_stamina=75,
            base_damage=17,
            special_move_name="Fireball"
        )
    
    def use_special_move(self, target):
        """Fireball: Ranged attack that ignores evade."""
        stamina_cost = 35
        if self.stamina < stamina_cost:
            return {'success': False, 'message': f"{self.name} doesn't have enough stamina!"}
        
        self.stamina -= stamina_cost
        
        # Fireball ignores evade
        was_evading = target.is_evading
        target.is_evading = False
        
        damage = int(self.base_damage * 2.2)  # ~37 damage
        
        # Apply damage with block check
        actual_damage, blocked_completely, damage_msg = self._apply_damage_with_block_check(target, damage, "Fireball")
        
        target.is_evading = was_evading
        
        if blocked_completely:
            return {
                'success': True,
                'damage': 0,
                'message': damage_msg,
                'stamina_cost': stamina_cost
            }
        else:
            return {
                'success': True,
                'damage': actual_damage,
                'message': f"{damage_msg} (ignores evade)",
                'stamina_cost': stamina_cost
            }


class Samurai(Character):
    """Precise, high accuracy, moderate damage."""
    
    def __init__(self):
        super().__init__(
            name="Samurai",
            max_hp=88,
            max_stamina=80,
            base_damage=16,
            special_move_name="Iaido Slash"
        )
    
    def use_special_move(self, target):
        """Iaido Slash: High accuracy, moderate damage."""
        stamina_cost = 28
        if self.stamina < stamina_cost:
            return {'success': False, 'message': f"{self.name} doesn't have enough stamina!"}
        
        self.stamina -= stamina_cost
        
        # High accuracy (90% hit rate even if evading)
        import random
        hit_chance = 0.9
        
        if target.is_evading and random.random() > hit_chance:
            return {'success': False, 'message': f"{target.name} evaded the attack!", 'stamina_cost': stamina_cost}
        
        damage = int(self.base_damage * 2.3)  # ~36-37 damage
        
        # Apply damage with block check
        actual_damage, blocked_completely, damage_msg = self._apply_damage_with_block_check(target, damage, "Iaido Slash")
        
        return {
            'success': True,
            'damage': actual_damage,
            'message': damage_msg,
            'stamina_cost': stamina_cost
        }


# Character registry for easy access
CHARACTERS = {
    'warrior': Warrior,
    'tank': Tank,
    'assassin': Assassin,
    'mage': Mage,
    'samurai': Samurai
}


def get_character(name):
    """
    Factory function to create a character instance by name.
    
    Args:
        name: Character name (case-insensitive)
        
    Returns:
        Character instance or None if not found
    """
    name_lower = name.lower()
    if name_lower in CHARACTERS:
        return CHARACTERS[name_lower]()
    return None


def list_all_characters():
    """Return a list of all available character names."""
    return list(CHARACTERS.keys())

